...
class MonsterIndex:
    def __init__(self, monsters, fonts, monster_frames):
        ...
        # superficie de tint para oscurecer el fondo cuando el índice está abierto
        ...
        # lista: configuración de scroll, elementos visibles, índices
        ...
        # preescalar iconos para tamaño consistente en la lista
        self.icon_size = max(24, min(64, int(self.item_height * 0.6)))
        self.scaled_icon_frames = {}
        for name, surf in self.icon_frames.items():
            try:
                self.scaled_icon_frames[name] = pygame.transform.smoothscale(surf, (self.icon_size, self.icon_size))
            except Exception:
                # fallback: superficie vacía con alpha si escala falla
                s = pygame.Surface((self.icon_size, self.icon_size), pygame.SRCALPHA)
                self.scaled_icon_frames[name] = s

        # calcular máximos de stats para normalizar barras
        self.max_stats = {}
    ...
    def input(self):
        ...
        if keys[pygame.K_SPACE]:
            if self.selected_index != None:
                ...
                # intercambio entre posiciones en la lista (reordenamiento sencillo)
    ...
    # la función display_list dibuja la lista lateral de monstruos
    def display_list(self):
        ...
        for index, monster in self.monsters.items():
            ...
            # icono (preescalado)
            icon_surf = self.scaled_icon_frames.get(monster.name)
            if not icon_surf:
                icon_surf = pygame.Surface((self.icon_size, self.icon_size), pygame.SRCALPHA)
                pygame.draw.rect(icon_surf, COLORS['dark'], icon_surf.get_rect(), border_radius=6)
            icon_rect = icon_surf.get_frect(center = item_rect.midleft + vector(45,0))

            if item_rect.colliderect(self.main_rect):
                # dibujar esquinas redondeadas en los extremos visibles
    ...
    # la función display_main dibuja el panel derecho con detalle del monstruo seleccionado
    def display_main(self, dt):
        # datos principales
        monster = self.monsters[self.index]

        # fondo principal del panel derecho
        ...
        # área superior con color según elemento
